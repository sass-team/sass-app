...
function countLabelsStatusAppointments($day, $i, $appointments) {
	...
	$canceledLabelMessagesForDay = array(Appointment::LABEL_MESSAGE_STUDENT_NO_SHOW,
		Appointment::LABEL_MESSAGE_TUTOR_CANCELED, Appointment::LABEL_MESSAGE_TUTOR_NO_SHOW, Appointment::LABEL_MESSAGE_STUDENT_CANCELED);
	$countCanceledAppointmentsForDay = Appointment::countWithLabelMessages($appointmentsForDay, $canceledLabelMessagesForDay);
	return array($weekOfYear, $year, $dateFormatted, $appointmentsForDay, $countAppointmentsForDay, $countAchievedAppointmentsForDay, $canceledLabelMessagesForDay, $countCanceledAppointmentsForDay);
}
...
<script type="text/javascript">
	$(function () {
		...
		if (!$('#workshop-chart').length) return false;
		if (!$('#area-chart-appointments').length) return false;
